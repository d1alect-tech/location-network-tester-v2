...
/** Убитые маршруты (A3): парсятся, но канонизируются в живые.
 * «Подготовка» была мёртвым плейсхолдером — входящие ссылки ведут на «Захват». */
export const LEGACY_ROUTE_ALIASES: Record<string, WorkspaceRoute> = {
  prepare: "capture",
};

/** Сырое имя маршрута из hash без валидации (нужно для legacy-редиректов). */
function rawRouteName(hash: string): string | null {
 ...
 * Убитый legacy-маршрут (см. LEGACY_ROUTE_ALIASES) отдаёт живой маршрут. */
export function parseHash(hash: string): WorkspaceLocation | null {
  const routePart = rawRouteName(hash);
  if (routePart === null) return null;
  const canonical = LEGACY_ROUTE_ALIASES[routePart] ?? routePart;
  if (!(WORKSPACE_ROUTES as readonly string[]).includes(canonical)) return null;
  const rest = hash.slice(2);
  const queryIndex = rest.indexOf("?");
  ...
  return { route: canonical as WorkspaceRoute, params: sanitizeParams(params) };
}
...
export class RouteStore {
   ...
   * Убитый legacy-маршрут в адресной строке один раз правится в канонический. */
  syncFromUrl(): void {
    const next = this.readLocation();
    const raw = rawRouteName(this.win.location.hash);
    if (raw !== null && LEGACY_ROUTE_ALIASES[raw] !== undefined) {
      this.location = next;
      this.win.location.hash = serializeLocation(next);
