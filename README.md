...
Одна ловушка, на которую легко наступить. API ищет сессию по имени каталога, а
каталог показывает идентификатор из манифеста. У синтетики идентификатор
собирается как `syn-<профиль>-seed<семя>`, поэтому каталог надо называть точно
так же. Назовёте иначе, сессия появится в списке, но спектр вернёт 404.
